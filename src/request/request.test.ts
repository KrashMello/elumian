...
describe("request", () => {
  ...
  it("should throw if the data is null", () => {
    expect(() => compareData(null)).toThrow("parameter data must be an object");
  });
  it("should throw if the optionsToValidate is null", () => {
    expect(() => compareData({ a: "" }, null)).toThrow(
      "parameter optionsToValidate must be an object",
    );
  });
  ...
  it("should return true if data.name lenght > 5 and lenght < 10", () => {
    expect(
      compareData({ name: "holaaaaaa" }, { name: ["min:5", "max:10"] }),
    ).toBe(true);
  });
  it("should return error if data.name is not Alpha", () => {
    expect(
      compareData({ name: "holaaa!$" }, { name: ["alpha"] }),
    ).toStrictEqual({
  ...
  it("should return error if data.name isAlphaSimbols", () => {
    expect(
      compareData({ name: "123" }, { name: ["alphaSimbol"] }),
    ).toStrictEqual({
  ...
  it("should return errors field password if not exists field in optionsToValidate", () => {
    expect(
      compareData({ password: "asdf" }, { name: ["min:5"] }),
    ).toStrictEqual({
  ...
  it("should return errors field must be boolean", () => {
    expect(
      compareData({ check: "true" }, { check: ["boolean"] }),
    ).toStrictEqual({
  ...
  it("should return errors field must be required", () => {
    expect(
      compareData(
        { check: true },
        { check: ["boolean"], password: ["required"] },
      ),
    ).toStrictEqual({
