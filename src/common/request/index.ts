...
export class validations {
	static dataCompare = {
		min: (value: string, length: number) => value && value.length >= length,
		max: (value: string, length: number) => value && value.length <= length,
			...
			.map((v: validationsOptionsFields) => {
				if (v.includes("min") || v.includes("max")) {
					...
					return this.dataCompare[type](data[key], Number(limit))
						? null
						: messages[type] + Number(limit);
				}
				if (!this.dataCompare[v]) {
					return messages.invalidField;
				}

				if (v === "required") messages[v] = messages[v].replace("<field>", key);

				return this.dataCompare[v](data[key]) ? null : messages[v];
			})
